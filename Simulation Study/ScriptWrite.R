...
################################
# Revision 2 Added Simulations #
################################
Ks <- c(5, 10, 20, 40, 80)
seeds <- c(401, 402, 403, 404, 406)
for(k in 1:length(Ks)) {
  set.seed(seeds[k])
  seeds <- sample(0:10000, 1800, replace = TRUE)
  for(iter in 1:1800) {
    filename <- paste0('~/Wake Forest University Dropbox/Noah Gade/Research/NPGC/Simulations/NPGC', Ks[k], '/Files/NPGC', Ks[k], '_', iter, '.R')
    fileConn <- file(filename)
    writeLines(c(paste0('source("/deac/sta/gadeGrp/gaden/NPGC/GC.R")'),
                 paste0('load("/deac/sta/gadeGrp/gaden/NPGC/Data/data.RData")'),
                 paste0('RNGkind("L', "'", 'Ecuyer-CMRG")'),
                 paste0('set.seed(', seeds[iter], ')'),
                 paste0('NPGC', Ks[k], '_', iter, ' <-  npgc(dat = data[[', iter, ']]$data, type = 0, hdim = 100, omega = 1, activation = 0, y_select = 1, z_select = 2, x_select = 3, max_lag = 2, m = 199, k = ', Ks[k], ', r = 25)'),
                 paste0('save(NPGC', Ks[k], '_', iter, ', file = "/deac/sta/gadeGrp/gaden/NPGC/NPGC', Ks[k], '/Results/NPGC', Ks[k], '_', iter, '.RData")'),
                 paste0('print("COMPLETE")')),
               fileConn)
    close(fileConn)
  }
  filename2 <- paste0('~/Wake Forest University Dropbox/Noah Gade/Research/NPGC/Simulations/NPGC', Ks[k], '/Files/NPGC', Ks[k], '.sh')
  fileConn2 <- file(filename2)
  writeLines(c('#!/bin/bash',
               '#SBATCH --account=gadeGrp',
               '#SBATCH --partition=small',
               ...
               paste0('#SBATCH --output=NPGC', Ks[k], '_%a.o'),
               '\n',
               'module load apps/r/4.3.3',
               paste0('Rscript NPGC', Ks[k], '_${SLURM_ARRAY_TASK_ID}.R')),
             fileConn2)
  close(fileConn2)
}
...
seeds <- sample(0:10000, 1800, replace = TRUE)
for(iter in 1:1800) {
  filename <- paste0('~/Wake Forest University Dropbox/Noah Gade/Research/NPGC/Simulations/NPGCn200/Files/NPGCn200_', iter, '.R')
    fileConn <- file(filename)
    writeLines(c(paste0('source("/deac/sta/gadeGrp/gaden/NPGC/GC.R")'),
                 paste0('load("/deac/sta/gadeGrp/gaden/NPGC/Data/data.RData")'),
                 paste0('RNGkind("L', "'", 'Ecuyer-CMRG")'),
                 paste0('set.seed(', seeds[iter], ')'),
                 paste0('NPGCn200_', iter, ' <-  npgc(dat = data[[', iter, ']]$data, type = 0, hdim = 200, omega = 1, activation = 0, y_select = 1, z_select = 2, x_select = 3, max_lag = 2, m = 199, k = 10, r = 25)'),
                 paste0('save(NPGCn200_', iter, ', file = "/deac/sta/gadeGrp/gaden/NPGC/NPGCn200/Results/NPGCn200_', iter, '.RData")'),
                 paste0('print("COMPLETE")')),
               fileConn)
    close(fileConn)
}
filename2 <- paste0('~/Wake Forest University Dropbox/Noah Gade/Research/NPGC/Simulations/NPGCn200/Files/NPGCn200.sh')
fileConn2 <- file(filename2)
writeLines(c('#!/bin/bash',
              '#SBATCH --account=gadeGrp',
              '#SBATCH --partition=small',
              ...
              '#SBATCH --mail-user=gaden@wfu.edu',
              '#SBATCH --mail-type=NONE',
              paste0('#SBATCH --output=NPGCn200_%a.o'),
              '\n',
              'module load apps/r/4.3.3',
              paste0('Rscript NPGCn200_${SLURM_ARRAY_TASK_ID}.R')),
            fileConn2)
close(fileConn2)